...
void enviaDadosPelaSerial(){              // opcional, para debug
  ...
  Serial.print(buffer[5],DEC); Serial.print("."); //Volt
  Serial.println(buffer[6],DEC); Serial.print(" "); //Volt
}
...
void funcaoDadoRecebido(int x){    // funcao chamada ao receber um dado
  ...
  if(dadoRecebido == 0x10){
     limiteAlarme = valorPot; //Define o valor do potenciometro como limite do alarme
     buffer[2] = buffer[0]; 
     buffer[3] = buffer[1];
